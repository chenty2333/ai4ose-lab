...
impl Schedule<ProcId> for ProcManager {
    ...
    fn fetch(&mut self) -> Option<ProcId> {
        ...
        for i in 1..self.ready_queue.len() {
            let a = self.tasks.get(&self.ready_queue[i]).unwrap().stride;
            let b = self.tasks.get(&self.ready_queue[min_idx]).unwrap().stride;
            if (a.wrapping_sub(b) as isize) < 0 {
                min_idx = i;
            }
        }
        let pid = self.ready_queue.remove(min_idx).unwrap();
        let proc = self.tasks.get_mut(&pid).unwrap();
