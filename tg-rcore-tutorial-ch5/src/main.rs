...
mod impls {
    ...
    impl Process for SyscallContext {
        ...
        fn spawn(&self, _caller: Caller, path: usize, count: usize) -> isize {
            ...
            current
                ...
                .and_then(|name| APPS.get(name).map(|elf_data| (name, elf_data)))
                .and_then(|(_name, elf_data)| ElfFile::new(elf_data).ok())
    ...
    impl Memory for SyscallContext {
        ...
        fn mmap(
            ...
            // 参数校验：地址页对齐、长度非零、prot 合法（1~7）
            if addr % PAGE_SIZE != 0 || len == 0 || prot == 0 || prot > 7 {
                return -1;
            }
            let mut flags: [u8; 5] = *b"U___V";
            if prot & 4 != 0 { flags[1] = b'X'; }
            if prot & 2 != 0 { flags[2] = b'W'; }
            if prot & 1 != 0 { flags[3] = b'R'; }
            let vm_flags = parse_flags(
                unsafe { core::str::from_utf8_unchecked(&flags) }
            ).unwrap();
        ...
        fn munmap(&self, _caller: Caller, addr: usize, len: usize) -> isize {
            ...
            while vpn < end_vpn.val() {
                let covered = current.address_space.areas.iter().any(|area| {
                    vpn >= area.start.val() && vpn < area.end.val()
                });
                if !covered {
                    return -1;
                }
