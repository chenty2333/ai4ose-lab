...
impl Inode {
    ...
    /// Increase nlink by 1 (for hard link)
    pub fn link(&self) {
        self.modify_disk_inode(|disk_inode| {
            disk_inode.nlink += 1;
        });
    ...
    /// Decrease nlink by 1, return true if should delete the file (nlink == 0)
    ...
    /// Get file stat (inode_id, mode, nlink)
    pub fn stat(&self) -> (u32, u32, u32) {
        ...
        self.read_disk_inode(|disk_inode| {
            ...
            (inode_id, mode, disk_inode.nlink)
        })
    }

    /// Create a directory entry pointing to an existing inode (for hard link)
    pub fn link_to(&self, name: &str, inode_id: u32) -> Option<()> {
        let mut fs = self.fs.lock();
        self.modify_disk_inode(|root_inode| {
            // Append dirent in the directory
            let file_count = (root_inode.size as usize) / DIRENT_SZ;
            let new_size = (file_count + 1) * DIRENT_SZ;
            // Increase size
            self.increase_size(new_size as u32, root_inode, &mut fs);
        ...
        self.modify_disk_inode(|root_inode| {
         ...
         let mut dirent = DirEntry::empty();
            ...
            for i in 0..file_count {
                ...
                if dirent.name() == name {
                    // Move the last dirent to this position
                    if i < file_count - 1 {
