...
mod impls {
    ...
    impl IO for SyscallContext {
       ...
       _flags: u32,
            ...
            let current = PROCESSOR.get_mut().current().unwrap();
       let old_string = if let Some(ptr) = current.address_space.translate(VAddr::new(oldpath), READABLE) {
              let mut s = String::new();
          let mut p: *mut u8 = ptr.as_ptr();
                loop { unsafe { let ch = *p; if ch == 0 { break; } s.push(ch as char); p = (p as usize + 1) as *mut u8; } }
           s
      } else { return -1; };
            let new_string = if let Some(ptr) = current.address_space.translate(VAddr::new(newpath), READABLE) {
            let mut s = String::new();
          let mut p: *mut u8 = ptr.as_ptr();
                loop { unsafe { let ch = *p; if ch == 0 { break; } s.push(ch as char); p = (p as usize + 1) as *mut u8; } }
                s
    ...
    /// unlinkat 系统调用：删除硬链接
       ...
       if let Some(file) = &current.fd_table[fd] {
            ...
            if let Some(inode) = &file.inode {
              ...
              .address_space
                        .translate::<Stat>(VAddr::new(st), WRITABLE)
             {
        let mut stat = Stat::new();
            stat.dev = 0;
             stat.ino = ino as u64;
                 stat.mode = StatMode::from_bits(mode).unwrap();
               stat.nlink = nlink;
